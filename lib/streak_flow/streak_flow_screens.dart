        ...
        children: [
          ...
          Padding(
            ...
            child: Column(
              children: [
                if (headerIcon != null) ...[
                  _streakPhotoHeaderIcon(headerIcon),
                  const SizedBox(height: 14),
                ],
                Expanded(
...
/// Display-only copy for the connection honesty card (UI labels 0–4).
/// Does not affect content-selection buckets or streak logic.
const List<List<String>> _kConnectionCardTitles = [
  // 0 Very Far
  [
    'God Welcomes You',
    'You’re Not Alone',
...
const List<List<String>> _kConnectionCardBodies = [
  // 0 Very Far
  [
    'God is closer than you think. Take one step toward Him today.',
    'No matter how distant you feel, His love never leaves you.',
    'You are always welcome in God’s presence.',
    'Today is a new opportunity to draw near to Him.',
    'God’s grace reaches you wherever you are.',
  ],
  // 1 Far
  [
    'Every step toward God is a step worth taking.',
    'Keep seeking Him. He is guiding your journey.',
    'God is patiently walking beside you today.',
    'Your faith is growing, one moment at a time.',
    'Stay open to His voice throughout your day.',
  ],
  // 2 Growing
  [
    'Your faith is taking root. Keep growing each day.',
    'God is shaping your heart through every step.',
    'Continue seeking Him with confidence.',
    'You’re moving forward in faith. Keep going.',
    'Stay faithful. God is working in your life.',
  ],
  // 3 Close
  [
    'You’re walking closely with God. Stay rooted in His Word.',
    'Keep drawing near to Him each day.',
    'Your relationship with God is growing stronger.',
    'Continue making time for His presence.',
    'May His peace guide you throughout today.',
  ],
  // 4 Very Close
  [
    'What a blessing to walk closely with God today.',
    'Remain faithful and let His light shine through you.',
    'Continue abiding in His presence every day.',
    'Keep your heart centered on Christ.',
    'May your closeness with God inspire those around you.',
  ],
];
...
// but we still map it into the existing 3 buckets for content selection.
  ...
  /// Display-only honesty card copy (does not affect content selection).
  String _cardTitle = '';
  String _cardBody = '';
  String? _lastCardCombo;
  ...
  void initState() {
    ...
    _refreshConnectionCardCopy();
    if (!widget.viewOnly) {
      _markStreakFlowStartedToday();
    }
  }

  /// Picks a random title + body for the current UI level.
  /// Avoids repeating the same title|body combo consecutively when possible.
  void _refreshConnectionCardCopy() {
    final level =
    _activeLabelIndex.clamp(0, _kConnectionCardTitles.length - 1).toInt();
    final titles = _kConnectionCardTitles[level];
    final bodies = _kConnectionCardBodies[level];
    if (titles.isEmpty || bodies.isEmpty) return;

    final rng = math.Random();
    var title = titles[rng.nextInt(titles.length)];
    var body = bodies[rng.nextInt(bodies.length)];
    var combo = '$title|$body';

    for (var i = 0; i < 8 && combo == _lastCardCombo; i++) {
      title = titles[rng.nextInt(titles.length)];
      body = bodies[rng.nextInt(bodies.length)];
      combo = '$title|$body';
    }

    _cardTitle = title;
    _cardBody = body;
    _lastCardCombo = combo;
  }
                                  ...
                                  children: [
                                    Slider(
                                      ...
                                      onChanged: widget.viewOnly
                                          ? null
                                          : (v) => setState(() {
                                        final prevLevel = _activeLabelIndex;
                                        final wasInteracted = _sliderInteracted;
                                        _sliderInteracted = true;
                                        _value = _snap(v);
// Display-only: refresh card copy when level
// changes or card first appears.
                                        if (!wasInteracted ||
                                            _activeLabelIndex != prevLevel) {
                                          _refreshConnectionCardCopy();
                                        }
                                      }),
                                    ),
              ...
              AnimatedSwitcher(
                ...
                child: _sliderInteracted && !widget.viewOnly
                    ? Padding(
                  key: ValueKey(
                      'honesty-card-$_activeLabelIndex-$_cardTitle'),
...
// Always show Save & Share (including after all steps completed).
                      _streakPhotoVerseSaveShareRow(
                        ...
                        onShare: () {
                          ...
                          const shareTitle = 'Verse of the Day';
                          showDialog(
                            context: shareContext,
                            builder: (dialogContext) => ShareAlertBox(
                              verseTitle: item.verseReference.isNotEmpty
                                  ? item.verseReference
                                  : shareTitle,
                              onShareAsText: () async {
                                  ...
                                  content: Column(
                                    ...
                                    children: [
                                      Text(
                                        shareTitle,
                                        textAlign: TextAlign.center,
                                        style: _streakShareBodyStyle().copyWith(
                                          fontSize: 22,
                                          fontWeight: FontWeight.w700,
                                        ),
                                      ),
                                      ...
                                      Text(
                                        '"${item.verseText}"',
                                        textAlign: TextAlign.center,
                                        style: _streakShareBodyStyle().copyWith(fontSize: 16),
                                      ),
                                      const SizedBox(height: 16),
                                      Text(
                                        item.verseReference,
...
// Always show Save & Share (including after all steps completed).
                      _streakPhotoSaveShareRow(
                        ...
                        onShare: () {
                          final shareContext = context;
                          const shareTitle = 'Devotional Moment';
                          const shareSubtitle =
                              'A short reflection to encourage your heart today.';
                          showDialog(
                            context: shareContext,
                            builder: (dialogContext) => ShareAlertBox(
                              verseTitle: shareTitle,
                              onShareAsText: () async {
                                Navigator.of(dialogContext).pop();
                                if (!shareContext.mounted) return;
                                await _shareText(
                                  ...
                                  content: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        shareSubtitle,
                                        textAlign: TextAlign.center,
                                        style: _streakShareReferenceStyle().copyWith(
                                          fontSize: 13,
                                        ),
                                      ),
                                      const SizedBox(height: 18),
...
// Always show Save & Share (including after all steps completed).
                      _streakPhotoSaveShareRow(
                        ...
                        onShare: () {
                          final shareContext = context;
                          const shareTitle = 'Prayer Moment';
                          const shareSubtitle = 'Talk to God and pour out your heart.';
                          showDialog(
                            context: shareContext,
                            builder: (dialogContext) => ShareAlertBox(
                              verseTitle: shareTitle,
...
// Restore completes a past day — record that day first.
// recordActivity() always writes today and breaks the
// consecutive count across the missed gap.
                            if (isRestoreRun) {
...
// One notif only after streak done: cancel streak nudges, prefer chat/verse.
                            ...
                            try {
                              ...
                              if (streakCount == 1) {
// Day-1 rating on LeaveRatingScreen — same open-ad defer as Day 2 until Home.
